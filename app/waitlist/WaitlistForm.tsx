...
const WaitlistForm = () => {
    ...
    if(message === "true") {
        ...
        if (firstNameElement && lastNameElement) {
            // Safely retrieve values from the input elements
            const firstName = (firstNameElement as HTMLInputElement).value;
            const lastName = (lastNameElement as HTMLInputElement).value;

            // Save to local storage
            localStorage.setItem('waitlist_referrer_first_name', firstName);
            localStorage.setItem('waitlist_referrer_last_name', lastName);
        } 
